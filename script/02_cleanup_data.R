...
survey_data_new <- survey_data %>%
  mutate(
    ...
    province = case_when(
      ...
      cps21_province == 6  ~ "Nova Scotia",
      cps21_province == 7  ~ "Ontario",
      cps21_province == 8  ~ "Prince Edward Island",
      cps21_province == 9  ~ "Quebec",
      cps21_province == 10 ~ "Saskatchewan",
      TRUE ~ NA_character_  
    ),
    education = case_when(
      cps21_education %in% c(1, 2, 3, 12) ~ "Low Education",
      cps21_education %in% c(4, 5, 6, 7, 8) ~ "Some Education",
      cps21_education %in% c(9, 10, 11) ~ "High Education"
    ),
    religion = ifelse(cps21_religion == 1, "NO", "YES"),
    income_before_tax = case_when(
      cps21_income_number >= 125000 ~ "Upper Class",
