...
Hence, our objective is to forecast the election probabilities for the Conservative and Liberal parties and compare which is higher[@Elections_2023]. The estimand of our research is relationship between age, gender, province, religion, income and education and probability of log odds of vote Conservative or Liberal party through binary logistic regression model.

## Terminology

In this paper, in @sec-Model, I used binary logistic regression models to forecast the likelihood of individuals affiliating with the Conservative or Liberal parties.Binary logistic regression is a statistical technique tailored for predicting binary outcomes, where the variable under consideration has two possible results.
In @sec-Model, these outcomes represent the probability of individuals aligning with each of the two parties.Each party's likelihood will be predicted through separate logistic regression models, with the response variables indicating whether individuals vote for the corresponding party or not.
The predictors used in these models include age, gender, province, education, religion, and income before tax is discussed in @sec-Data.
This regression approach is particularly advantageous when there's a nonlinear relationship between independent and dependent variables, especially in scenarios with categorical dependent variables like ours, details can be found in @sec-Data.
Additionally, in @sec-Model, we will incorporate post-stratification in our analysis to enhance accuracy.
...
We utilize this technique to predict the probability of voter shares for the Liberal or Conservative parties in the upcoming election in @sec-Result.
Finally, the consideration of potential bias and limitations is discussed in @sec-Discussion.
...
**Age**: In the census data, age is presented as a decimal number.
For consistency with the survey data, round this value to the nearest whole number.

**Gender**: The gender classification in the census data, listed as either 'Male' or 'Female', aligns perfectly with the survey data, requiring no modifications.

**Province**: The listing of provinces in the census data, including "Newfoundland" and "Labrador", "Nova Scotia", "Quebec", "Saskatchewan", "Ontario", "Alberta", "British Columbia", "Prince Edward Island", "New Brunswick", and "Manitoba", matches that of the survey data, making no further adjustments necessary.

**Educational**: The census data sorts educational attainment into eight categories, with one category for missing information.
...
**Religious**: The "religion_has_affiliation" variable in the census data identifies if an individual has a religious affiliation, is uncertain, claims no religious affiliation, or if the response is missing.
...
**Income**: The Income_respondent field in the census dataset uses 7 income brackets to classify respondents' earnings.
...
Founded at the inception of Canada in 1867, the Liberal Party has long been a prominent player in the nation's political arena. Rooted in the principles of liberalism—a cornerstone of Canadian political ideology—the party champions a balanced fiscal approach, advocating for judicious tax reductions to foster economic expansion, boost investment, and invigorate consumer spending. At the heart of the Liberal Party's platform is a commitment to economic vitality and job creation, with a focus on propelling Canada's growth and ensuring ample employment opportunities. The party also prioritizes the well-being of its constituents, pledging to sustain and enhance public services including healthcare, pensions, and education. Committed to environmental stewardship, the Liberal Party envisions a greener Canada, aiming for reduced carbon emissions and heightened conservation efforts.
Our analysis, drawn from five different polls, suggests a marginal preference for the Liberal Party over the Conservative Party.
This trend is echoed in Russell's 2023 article, which indicates a public perception of the Liberal Party's superior handling of these key issues.
Thus, our working hypothesis posits that the Liberal Party holds a slender advantage in the race for the upcoming election[@russell2023two].
...
After refining the models using Akaike's Information Criteria to eliminate superfluous predictors, we established age, gender, province, religion, education, and pre-tax income as our final variables.

Post-stratification was applied to these predictors, revealing that the Liberal Party has a projected probability of 28.83%, marginally outpacing the Conservative Party's 28.66%.
These projections reinforce our initial hypothesis that the Liberal Party is the slight favorite for the 2025 election, based on our analysis of the data at hand.
...
The nearly identical yPS values for the Conservative and Liberal parties, standing at 0.2865 and 0.2883 respectively, suggest a possibility of incomplete or inaccurate data processing. Such closeness in predictive scores could point to calculation errors during the yPS estimation, which in turn could impact the determination of the potential election winner.
...
Aligning with Russell’s analysis and multiple polls, our findings suggest the Liberal Party is poised for a narrow victory over the Conservative Party, informed by an intricate blend of sociopolitical factors and current Canadian sentiment[@russell2023two].
...
#| label: fig-liberal-votes
#| fig-cap: "Posterior and Perior check for Liberal Party"
#| echo: false
...
```
@fig-liberal-votes shows that there is no big difference between posterior and prior model of liberal party. When discussing prior and posterior in the context of Bayesian statistics, the prior distribution represents our initial beliefs about the parameters before observing the data, while the posterior distribution is an updated belief that takes the observed data into account. The closer the posterior is to the prior, the less influence the observed data has had on our beliefs. This could mean that the data was consistent with our expectations or that the data wasn't particularly informative with respect to that parameter.

If we were to infer the relationship for the Liberal Party, without direct data it would be speculative. However, if the model parameters included variables that could be linked to political tendencies or support for the Liberal Party (such as demographics or location), one might analyze the posterior distributions of these parameters to determine how the observed data aligns with the initial assumptions. If the posterior distributions are very similar to the prior distributions, it would suggest that the data confirmed the initial beliefs about these factors' relationship to support for the Liberal Party without significant changes. This indicates that the model's predictions for how certain factors influence support for the Liberal Party were quite accurate to begin with.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.width=8, fig.height=6}
#| label: fig-conservative-votes
#| fig-cap: "Posterior and Perior check for Conservative Party"
#| echo: false
prior_covariates <- normal(location = 0, scale = 2.5, autoscale = FALSE)
prior_intercept <- normal(0, 2.5)
...
@fig-conservative-votes compares the posterior and prior estimates of parameters for a Bayesian hierarchical model, analyzing factors that influence votes for the conservative party. The left side  shows the estimated values of parameters after the model has been updated with the data; these are the revised beliefs about the parameters' values. The colors and positions of the dots represent the mean or median of the posterior distributions, and the vertical lines may indicate the uncertainty or credible intervals. The right side suggesting it would normally show the initial beliefs or assumptions about the parameters before any data was observed; these priors are specified by the analyst based on prior knowledge or assumptions. The lack of information on the "Prior" side indicates that this visualization is focusing on the posterior outcomes. Essentially, the relationship between posterior and prior is that the posterior is the updated belief after considering the evidence (data), whereas the prior represents the beliefs before seeing the data. The posterior is informed by and is an adjustment from the prior, incorporating the information gained from the observed data.
