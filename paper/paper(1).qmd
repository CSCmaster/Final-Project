...
# Perform stepwise model selection using stepAIC
step_cons <- stepAIC(initial_model, direction="both", trace = FALSE)  

# Use broom::tidy to get a tidy data frame of the model summary with confidence intervals
tidy_cons <- broom::tidy(step_cons, conf.int = TRUE)

# Select the columns you want for the table, excluding statistic and p-value
tidy_cons_selected <- tidy_cons[, c("term", "estimate", "std.error", "conf.low", "conf.high")]

# Create the table with knitr::kable()
kable_output <- knitr::kable(tidy_cons_selected, format = "html", caption = "Model Summary for Conservative Vote Prediction with Confidence Intervals")
...
# Assuming 'vote_liberal' is the column in your 'cleaned_survey_data' that represents voting for liberal
...
kable_output_lib <- knitr::kable(tidy_lib_selected, format = "html", caption = "Model Summary for Liberal Vote Prediction with Confidence Intervals")
kable_output_lib
...
@tbl-coef-summary-liberal

`

@tbl-coef-summary-conservative
...
# Assuming you have already fitted a model `step_lib`
# and `step_lib` is an object of class 'glm' or similar
tidy_lib <- broom::tidy(step_lib, conf.int = TRUE, exponentiate = FALSE)  # Set exponentiate=TRUE for logistic regression

# Forest plot of coefficient estimates
ggplot(tidy_lib, aes(x = estimate, y = reorder(term, estimate))) +
  geom_point() +
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high), width = 0.2) +
  geom_vline(xintercept = 1, linetype = "dotted", color = "red") +  # Use 0 if exponentiate=FALSE
  labs(x = "Estimate", y = "Coefficient", title = "Coefficient Estimates for Liberal Party") +
  theme_minimal()
# Assuming your `cleaned_survey_data` has a column `province` and `vote_liberal`
proportions_by_province <- cleaned_survey_data %>%
  group_by(province) %>%
  summarise(estimate = mean(vote_liberal)) %>%
  mutate(province = factor(province, levels = rev(unique(province))))  # Ordering the provinces

# Horizontal bar plot of estimated proportions by province
ggplot(proportions_by_province, aes(x = province, y = estimate)) +
  geom_col() +
  coord_flip() +  # Flipping the coordinates for horizontal bars
  labs(y = "Estimated Proportion", x = "Province", title = "Estimated Proportion of Voting for Liberal by Province") +
  theme_minimal()
...
cleaned_survey_data$vote_liberal <- ifelse(cleaned_survey_data$vote_liberal == "Yes", 1, 0)

lib_props <- cleaned_survey_data %>% 
...
library(ggplot2)
library(dplyr)
# Assuming 'cleaned_survey_data' is your data frame and it's already loaded in R
# First, check that all required variables are present in 'cleaned_survey_data'
required_vars <- c("vote_conservative", "age", "sex", "province", "education", "religion", "income_before_tax")
...
# If all variables are present, fit the linear model
model <- lm(vote_conservative ~ age + sex + province + education + religion + income_before_tax, data=cleaned_survey_data)
...
ggplot(model_summary, aes(x = estimate, y = reorder(term, estimate))) +
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Estimate", y = "Coefficient", title = "Coefficient Estimates") +
  theme_minimal()
...
# Assuming 'your_data' is your dataframe and it contains the variables 'province' and 'vote_conservative'
# where 'vote_conservative' is a binary (0/1) indicating whether a vote for conservative was cast.

# Calculate proportions and confidence intervals by province
conservative_votes_by_province <- cleaned_survey_data %>%
  group_by(province) %>%
  summarise(
    proportion = mean(vote_conservative, na.rm = TRUE),
    lower = proportion - qnorm(0.975) * sqrt(proportion * (1 - proportion) / n()),
    upper = proportion + qnorm(0.975) * sqrt(proportion * (1 - proportion) / n())
  )

# Create the plot
ggplot(conservative_votes_by_province, aes(x = reorder(province, -proportion), y = proportion)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  coord_flip() +  # Flip the axes to have provinces on the y-axis
  labs(x = "Proportion", y = "Province", title = "Estimated Proportion of Voting for Conservative by Province") +
  theme_minimal()
...
conservative_votes_by_education <- cleaned_survey_data %>%
  group_by(education) %>%
  summarise(
    proportion = mean(vote_conservative, na.rm = TRUE),
    lower = proportion - qnorm(0.975) * sqrt(proportion * (1 - proportion) / n()),
...
ggplot(conservative_votes_by_education, aes(x = reorder(education, proportion), y = proportion)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  coord_flip() +  
  labs(x = "Proportion", y = "Education", title = "Proportion of Votes for Conservative by Education Level") +
  theme_minimal()
...
In conclusion, our binary logistic regression approach, supplemented with post-stratification, has yielded a data-driven forecast for the 2025 Canadian Election. Aligning with Barik’s analysis and multiple polls, our findings suggest the Conservative Party is poised for a narrow victory over the Liberal Party, informed by an intricate blend of sociopolitical factors and current Canadian sentiment[@Barik_2023].
...
# Generate the posterior vs. prior plot for the conservative model
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Make sure 'sex' is part of the group_by when creating step_lib_counts
step_lib_counts <- cleaned_survey_data %>%
  group_by(age, sex, province, education, religion, income_before_tax) %>%
  summarise(N_j=n(), .groups = 'drop')  # Use .groups = 'drop' to remove the grouping

# Now the step_lib_counts has a 'sex' column that matches with the model's predictors
# You can proceed with prediction
lib_predictions <- predict(step_lib, newdata = step_lib_counts, type = "response")

# After prediction, if you want to calculate confidence intervals manually, make sure
# the calculations are vectorized and account for the grouped structure of your data

# Assuming that N_j is the count of observations for each group in your summarized dataset
...
# Here we don't need to create an 'observation' column because the plot will use grouped data
# If you do want an 'observation' identifier, ensure it's a sequence within each group
